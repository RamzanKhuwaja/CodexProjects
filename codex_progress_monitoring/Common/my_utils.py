...
# Resolve project root from the Common package directory.
...
VAU_CLASS_MAP_FILE  = os.path.join(_PROJ_ROOT, 'Common', 'VAUClassMap2025-26.csv')



MAE_CLASS_MAP_FILE  = os.path.join(_PROJ_ROOT, 'Common', 'MAEClassMap2025-26.csv')



VAU_STUDENT_MAP_FILE = os.path.join(_PROJ_ROOT, 'Common', 'VAUStudentMap2025-26.csv')



MAE_STUDENT_MAP_FILE = os.path.join(_PROJ_ROOT, 'Common', 'MAEStudentMap2025-26.csv')
...
def check_duplicates_in_column(df, column_name) -> TableData | None:
    if PANDAS_AVAILABLE and hasattr(df, "duplicated"):
        ...
        if not duplicates.empty:  # type: ignore[union-attr]
            ...
            base_columns = [str(col) for col in duplicates.columns]  # type: ignore[union-attr]
            for index, row in duplicates.iterrows():  # type: ignore[union-attr]
                print(f"Row {index + 2}: {row[column_name]}")
                duplicate_row: dict[str, object] = {
                    "Duplicate Column": column_name,
                    "Row Number": index + 2,
                }
                for col in base_columns:
                    value = row[col]
                    duplicate_row[col] = "" if is_missing(value) else value
                duplicate_rows.append(duplicate_row)

            print()
            return TableData(["Duplicate Column", "Row Number", *base_columns], duplicate_rows)
        else:
    ...
    for index, row in enumerate(df):
        ...
        for key in row.keys():
            key_str = str(key)
            if key_str not in base_columns:
                base_columns.append(key_str)
    ...
    if duplicates:
        ...
        for index, value in duplicates:
            ...
            source_row = df[index] if 0 <= index < len(df) else {}
            duplicate_row: dict[str, object] = {
                "Duplicate Column": column_name,
                "Row Number": index + 2,
            }
            if isinstance(source_row, Mapping):
                for col in base_columns:
                    duplicate_row[col] = source_row.get(col, "")
            duplicate_rows.append(duplicate_row)
        print()
        return TableData(["Duplicate Column", "Row Number", *base_columns], duplicate_rows)
    else:
...
def check_class_map(
    ...
    if PANDAS_AVAILABLE:
        ...
        if df.empty:  # type: ignore[union-attr]
            print(f"ERROR: No data rows found in class map: {class_map}")
            return False
    ...
    if not fieldnames:
        print(f"ERROR: Class map file has no header row: {class_map}")
        return False
    if not rows:
        print(f"ERROR: No data rows found in class map: {class_map}")
        return False
    ...
    for column in columns_to_check:
        if column in fieldnames:
            duplicates = check_duplicates_in_column(rows, column)
            if duplicates is not None:
                has_duplicates = True
                if collect_duplicates is not None:
                    collect_duplicates.append(duplicates)
...
def _build_class_rosters(student_map: pd.DataFrame) -> dict[str, set[str]]:
    rosters: dict[str, set[str]] = {}
    if student_map.empty:
        return rosters
    required_columns = {'Class Code', 'Org Defined ID'}
    if not required_columns.issubset(student_map.columns):
        return rosters

    cleaned = student_map[['Class Code', 'Org Defined ID']].copy()
    cleaned['Class Code'] = cleaned['Class Code'].astype(str).str.strip()
    cleaned['Org Defined ID'] = cleaned['Org Defined ID'].astype(str).str.strip()
    cleaned = cleaned[
        (cleaned['Class Code'] != '')
        & (cleaned['Class Code'].str.lower() != 'nan')
        & (cleaned['Org Defined ID'] != '')
        & (cleaned['Org Defined ID'].str.lower() != 'nan')
    ]

    for class_code, group in cleaned.groupby('Class Code'):
        rosters[class_code] = set(group['Org Defined ID'])
    return rosters


def _infer_class_code_from_student_ids(
    student_ids: set[str],
    class_rosters: Mapping[str, set[str]] | None,
    context: str,
) -> Optional[str]:
    if not student_ids or not class_rosters:
        return None

    overlaps: list[tuple[int, str]] = []
    for class_code, roster_ids in class_rosters.items():
        overlap_count = len(student_ids & roster_ids)
        if overlap_count > 0:
            overlaps.append((overlap_count, class_code))

    if not overlaps:
        warn_once("WARNING", f"Could not infer class code for '{context}' from student roster overlap")
        return None

    max_overlap = max(overlap for overlap, _ in overlaps)
    matches = sorted(class_code for overlap, class_code in overlaps if overlap == max_overlap)
    if len(matches) > 1:
        warn_once(
            "WARNING",
            f"Ambiguous class code for '{context}' from student roster overlap: {matches}",
        )
        return None
    return matches[0]


def _select_latest_class_exports(
    entries: list[tuple[str, float, pd.DataFrame]],
    data_label: str,
) -> list[pd.DataFrame]:
    chosen_by_class: dict[str, tuple[str, float, pd.DataFrame]] = {}

    for class_code, modified_time, frame in entries:
        file_name = os.path.basename(str(frame.attrs.get('source_file', '')))
        existing = chosen_by_class.get(class_code)
        if existing is None:
            chosen_by_class[class_code] = (file_name, modified_time, frame)
            continue

        existing_name, existing_mtime, existing_frame = existing
        existing_rows = len(existing_frame)
        current_rows = len(frame)

        keep_current = (
            modified_time > existing_mtime
            or (
                modified_time == existing_mtime
                and (current_rows > existing_rows or (current_rows == existing_rows and file_name > existing_name))
            )
        )
        ...
        if keep_current:
            warn_once(
                "WARNING",
                f"Multiple {data_label} exports matched class code '{class_code}'. Keeping latest file '{file_name}'.",
            )
            chosen_by_class[class_code] = (file_name, modified_time, frame)
        else:
            warn_once(
                "WARNING",
                f"Multiple {data_label} exports matched class code '{class_code}'. Keeping latest file '{existing_name}'.",
            )

    return [entry[2] for entry in chosen_by_class.values()]


def get_attendance_data(
    attendance_dir: str,
    class_rosters: Mapping[str, set[str]] | None = None,
) -> pd.DataFrame:
    ...
    for filename in csv_paths:
        ...
        student_ids = {student_id.strip() for student_id in df['Org Defined ID'].astype(str) if student_id.strip()}
        ...
        entries.append((class_code, os.path.getmtime(filename), df))

    if not entries:
        print("No attendance data found")
        return pd.DataFrame()

    frames = _select_latest_class_exports(entries, "attendance")
...
def get_grades_data(
    ...
    for filename in csv_paths:
        ...
        result.attrs['source_file'] = filename
        entries.append((class_code, os.path.getmtime(filename), result))

    if not entries:
        print("No grades data found")
        return pd.DataFrame()

    frames = _select_latest_class_exports(entries, "grades")
...
def FindDupStudentsInBSViaClassList(
    ...
    if student_table.is_empty:
        print("ERROR: No student data found in any of the HTML files.")
        return False
...
def FindDupStudentsInBSViaAttendanceGrades(
    ...
    *,
    ...
    if not csv_files:
        ...
        print(f"ERROR: No CSV files found in directory: {target_dir}")
        return False
    ...
    if not records:
        print(f"ERROR: No files with required columns found in directory: {target_dir}")
        return False
...
def GenerateStudentMap(campus):
    ...
    if StudentMap.empty:
        ...
        return False
    duplicate_student_rows = StudentMap.duplicated(['Org Defined ID', 'Class Code'], keep='first')
    duplicate_count = int(duplicate_student_rows.sum())
    if duplicate_count:
        warn_once(
            "WARNING",
            f"Removing {duplicate_count} duplicate class-list rows using (Org Defined ID, Class Code)",
        )
        StudentMap = StudentMap.loc[~duplicate_student_rows].reset_index(drop=True)
    ...
    class_rosters = _build_class_rosters(StudentMap)

    print()
    print("Start - Getting attendance data")
    attendance_df = get_attendance_data(attendance_dir, class_rosters)
    print("End - Getting attendance data")

    attendance_lookup: dict[tuple[str, str], dict] = {}
    lesson_week = max(THIS_WEEK_NUM - 2, 1)
    lesson_col = f"Lesson {lesson_week}"

    if not attendance_df.empty:
        required_attendance_cols = {'Class Code', 'Org Defined ID'}
        if required_attendance_cols.issubset(attendance_df.columns):
            attendance_df['Org Defined ID'] = attendance_df['Org Defined ID'].astype(str)
            attendance_df['Class Code'] = attendance_df['Class Code'].astype(str)
            attendance_lookup = (
                attendance_df
                .drop_duplicates(['Class Code', 'Org Defined ID'])
                .set_index(['Class Code', 'Org Defined ID'])
                .to_dict('index')
            )
        else:
            warn_once("WARNING", "Attendance data missing 'Class Code' or 'Org Defined ID' columns")
    else:
    ...
    student_keys = {
        (str(class_code).strip(), str(student_id).strip())
        for class_code, student_id in zip(StudentMap['Class Code'], StudentMap['Org Defined ID'])
        if str(class_code).strip() and str(student_id).strip()
    }
    attendance_keys = set(attendance_lookup.keys()) if attendance_lookup else set()

    if attendance_keys:
        missing_students = student_keys - attendance_keys
        extra_attendance = attendance_keys - student_keys

        if missing_students:
            sample_missing = sorted(missing_students)[:5]
            warn_once(
                "WARNING",
                f"{len(missing_students)} class roster rows missing attendance entries (e.g., {sample_missing})",
            )
        ...
        if extra_attendance:
            ...
            warn_once(
                "WARNING",
                f"{len(extra_attendance)} attendance records do not match any class roster row (e.g., {sample_extra})",
            )
    ...
    for index, row in StudentMap.iterrows():
        ...
        lookup_key = (str(class_code).strip(), str(student_id).strip())
        if all(lookup_key) and lookup_key in attendance_lookup:
            matching_row = attendance_lookup[lookup_key]
    ...
    grades_df = get_grades_data(grades_dir, class_rosters)
    print("End - Getting grade data")

    grade_lookup: dict[tuple[str, str], dict] = {}
    if not grades_df.empty:
        required_grade_cols = {'Class Code', 'OrgDefinedId'}
        if required_grade_cols.issubset(grades_df.columns):
            grades_df['OrgDefinedId'] = grades_df['OrgDefinedId'].astype(str)
            grades_df['Class Code'] = grades_df['Class Code'].astype(str)
            grade_lookup = (
                grades_df
                .drop_duplicates(['Class Code', 'OrgDefinedId'])
                .set_index(['Class Code', 'OrgDefinedId'])
                .to_dict('index')
            )
        else:
            warn_once("WARNING", "Grade data missing 'Class Code' or 'OrgDefinedId' columns")
    else:
    ...
    if grade_ids:
        missing_grades = student_keys - grade_ids
        extra_grades = grade_ids - student_keys

        if missing_grades:
            sample_missing = sorted(missing_grades)[:5]
            warn_once(
                "WARNING",
                f"{len(missing_grades)} class roster rows missing grade entries (e.g., {sample_missing})",
            )
        ...
        if extra_grades:
            ...
            warn_once(
                "WARNING",
                f"{len(extra_grades)} grade records do not match any class roster row (e.g., {sample_extra})",
            )
    ...
    for index, row in StudentMap.iterrows():
        ...
        class_code = row['Class Code']
        lookup_key = (str(class_code).strip(), str(student_id).strip())
        if all(lookup_key) and lookup_key in grade_lookup:
            matching_row = grade_lookup[lookup_key]
...
def FindMissingAttendance(campus):
    ...
    df_missing = df_student_map[df_student_map['Att Uptodate?'] == False].copy()
    if df_missing.empty:
        return pd.DataFrame(columns=list(required_cols))

    # If the same student appears in multiple classes and one class row already has
    # attendance data, suppress the other class rows that are missing attendance
    # entirely. Those rows are usually duplicate-enrollment artifacts and should be
    # handled by office cleanup instead of teacher-facing reminders.
    has_attendance_by_student = (
        df_student_map
        .assign(
            _student_id=df_student_map['Org Defined ID'].astype(str).str.strip(),
            _has_attendance=df_student_map['Attendance (%)'].notna(),
        )
        .groupby('_student_id')['_has_attendance']
        .any()
    )
    duplicate_class_counts = (
        df_student_map
        .assign(_student_id=df_student_map['Org Defined ID'].astype(str).str.strip())
        .groupby('_student_id')['Class Code']
        .nunique()
    )
    df_missing['_student_id'] = df_missing['Org Defined ID'].astype(str).str.strip()
    ambiguous_mask = (
        df_missing['Attendance (%)'].isna()
        & df_missing['_student_id'].map(duplicate_class_counts).fillna(1).gt(1)
        & df_missing['_student_id'].map(has_attendance_by_student).fillna(False)
    )
    ambiguous_count = int(ambiguous_mask.sum())
    if ambiguous_count:
        warn_once(
            "WARNING",
            f"Suppressing {ambiguous_count} missing-attendance reminder rows caused by duplicate multi-class enrollments",
        )
        df_missing = df_missing.loc[~ambiguous_mask].copy()
    if df_missing.empty:
        return pd.DataFrame(columns=list(required_cols))
    ...
    df_result = df_result.drop_duplicates(['Org Defined ID', 'Class Code']).reset_index(drop=True)
    df_result['Start Week'] = pd.to_numeric(df_result['Start Week'], errors='coerce').fillna(-1).astype(int)
    return df_result


def email_att_missing_to_stakeholders(df_missing_attendance: pd.DataFrame) -> bool:
    ...
    for email in df_missing_attendance['Teacher Email'].dropna().unique():
        ...
        ]].copy().drop_duplicates().sort_values(['Class Code', 'Student Full Name', 'Org Defined ID'])
        ...
        print(
            f"Preparing missing attendance email for {teacher} "
            f"({len(df_report)} students) -> To: {to}; CC: {cc or '(none)'}"
        )
        ...
        if not send_email(to, cc, subject_email, body_email):
            all_success = False
