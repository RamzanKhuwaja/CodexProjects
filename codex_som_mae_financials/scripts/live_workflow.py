"""
live_workflow.py - MAE live Codex workflow helpers
...
def read_pl_compare(path: Path):
    ...
    for row in rows[:8]:
        ...
        for cell in row:
            if isinstance(cell, str) and " - " in cell and ("2026" in cell or "2025" in cell):
                parts = cell.split(" - ")
                if len(parts) == 2:
                    cutoff_date = parts[1].replace("(PY YTD)", "").replace("(custom)", "").strip()
                    break
        if cutoff_date != "Unknown":
            break
    ...
    taxes_booked = get("8500 Taxes Paid")["cy"]
    taxes_booked_prior = get("8500 Taxes Paid")["py"]
    carbon_rebate = get("Canada Carbon Rebate (Non-Taxable)")["cy"]
    carbon_rebate_prior = get("Canada Carbon Rebate (Non-Taxable)")["py"]
    ...
    return {
        ...
        "taxes_booked": taxes_booked,
        "taxes_booked_prior": taxes_booked_prior,
        "canada_carbon_rebate": carbon_rebate,
        "canada_carbon_rebate_prior": carbon_rebate_prior,
    }
...
def read_pl_3yr(path: Path):
    ...
    return {
        ...
        "ftc_total": round(data.get("6201.1 FTC", 0.0), 2),
...
def summarize_shareholder_items(ramzan_transactions, rezai_transactions):
    hajj_total = 0.0
    je11_entries = []
    je12_entries = []

    for tx in ramzan_transactions:
        memo = (tx.get("memo") or "").strip().lower()
        if "hajj" in memo:
            hajj_total += abs(tx["amount"])
        if tx.get("num") == "JE-11":
            je11_entries.append(tx)

    for tx in rezai_transactions:
        if tx.get("num") == "JE-12":
            je12_entries.append(tx)

    return {
        "hajj_total": round(hajj_total, 2),
        "je11_entries": je11_entries,
        "je12_entries": je12_entries,
    }
...
def build_live_session_packet():
    ...
    sh_ramzan_cy, sh_rezai_cy = read_shareholder_file(required["sh_current"])
    sh_ramzan_all, sh_rezai_all = read_shareholder_file(required["sh_all"])
    ...
    net_shareholder = round(sh_ramzan_cy["closing_balance"] + sh_rezai_cy["closing_balance"], 2)
    shareholder_flags = summarize_shareholder_items(
        sh_ramzan_cy["transactions"],
        sh_rezai_cy["transactions"],
    )
    ...
    packet = {
        "meta": {
            ...
            "workflow": "MAE live Codex session",
            "company": "Spirit of Math Schools Markham East",
        ...
        "evidence": {
            ...
            "marketing": {
                ...
                "ftc_current": round(pl_compare["raw_accounts"].get("6201.1 FTC", {}).get("current_ytd", 0.0), 2),
            },
            "tax": {
                "qb_profit_current_ytd": round(qb_profit, 2),
                "corporate_tax_proxy_component": round(taxes_booked, 2),
            ...
            "shareholder": {
                ...
                "rezai_closing": sh_rezai_cy["closing_balance"],
                "net_current_balance": net_shareholder,
                "hajj_total_in_ramzan_account": shareholder_flags["hajj_total"],
                "je11_entries": shareholder_flags["je11_entries"],
                "je12_entries": shareholder_flags["je12_entries"],
            },
            "historical_context": {
                ...
                "shareholder_all_dates": {
                    ...
                    "rezai": {
                        "opening_balance": sh_rezai_all["opening_balance"],
                        "closing_balance": sh_rezai_all["closing_balance"],
                    },
                },
            },
        },
        ...
        "codex_brief_prompts": {
            ...
            "shareholder": [
                "What is the real current shareholder position?",
...
def render_report_bundle(payload_path: Path):
    ...
    for topic in CORE_REPORT_TOPICS:
        ...
        output_path = BASE_DIR / "reports" / f"codex_live_report_{topic}_mae_{meta['report_date']}.docx"
