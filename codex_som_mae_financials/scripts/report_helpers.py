"""
report_helpers.py — Shared python-docx styling helpers for all som_mae_financials report scripts.
...
def sdat(row, bold=False):
    """Style a table data row (borders only)."""
    for cell in row.cells:
        sbd(cell)
        for para in cell.paragraphs:
            for run in para.runs:
...
# ── Callout boxes ──────────────────────────────────────────────────────────────
def callout(doc, text, color_fill="FFF3CD", text_color=None):
    """
    Parametric callout box (single string).
    Default: yellow background, brown text.
    Use color_fill (hex string) and text_color (RGBColor) to customise.
    """
