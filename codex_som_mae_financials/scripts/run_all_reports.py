"""
run_all_reports.py — som_mae_financials
========================================
...
# Keywords that must each match at least one file in data/current/ (.xlsx or .csv)
REQUIRED_KEYWORDS = [
...
def run(script_rel, label):
    ...
    result = subprocess.run(
        ...
        cwd=BASE_DIR,
        capture_output=False,   # output flows directly to terminal
    )
...
def fmt_currency(value):
    """Format a float as a dollar amount with sign for negatives."""
    if value is None:
        return "n/a"
    if value < 0:
...
def print_data_snapshot():
    ...
    rezai    = sh.get("rezai",  {}).get("closing_balance")
    ...
    if ramzan is not None:
        label = "owes corp" if ramzan < 0 else "owed by corp"
        print(f"  Ramzan:       {fmt_currency(ramzan)}  ({label})")
    if rezai is not None:
        label = "owes corp" if rezai < 0 else "owed by corp"
        print(f"  Rezai:        {fmt_currency(rezai)}  ({label})")
    print(line)
...
def main():
    ...
    print()
    print("=" * 62)
    print("  SOM MAE FINANCIALS — FULL PIPELINE")
    ...
    for script_rel, label in REPORT_SCRIPTS:
        ...
        # After the extract step, print the data snapshot
        if "extract_data" in script_rel:
            print_data_snapshot()

    # Step 8: validate (optional — skipped if validate_all.py not yet written)
    ...
    if os.path.exists(validate_abs):
        ...
        print("        Create the script and re-run to enable auto-validation.")
    ...
    # Collect report filenames from reports/
    ...
    print(banner_line(f"Reports saved in: reports/"))
