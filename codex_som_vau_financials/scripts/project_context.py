"""
Shared project context helpers for VAU reporting.
...
PROJECT_NAME = "Spirit of Math Schools Vaughan"
...
def load_review_fs_text(base_dir: Path) -> str:
    archive_dir = base_dir / "data" / "archive"
    pdf_path = first_match(archive_dir, "*REVIEW*2025*.pdf", "*July31_2025*.pdf", "*2025*.pdf")
    if pdf_path is None:
...
def parse_sbd_limit(requirements_text: str) -> float:
    ...
    if value is None:
        value = money_from_text(requirements_text, r"it's \$\s*([0-9,]+) for each")
    if value is None:
